...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        if load_img:
            self.imgs = [imread_unicode(x) for x in self.img_names]
            self.width = self.imgs[0].shape[1]
            self.height = self.imgs[0].shape[0]
        else:
            img = imread_unicode(self.img_names[0])
    ...
    def __getitem__(self, idx):
        gt = self.gt_traj[idx] if idx < len(self.gt_traj) else self.gt_traj[-1]
        if self.imgs is None:
            return imread_unicode(self.img_names[idx]), gt
        else:
            return self.imgs[idx], gt

    def __iter__(self):
        for i in range(len(self.img_names)):
            if self.imgs is not None:
                gt = self.gt_traj[i] if i < len(self.gt_traj) else self.gt_traj[-1]
            else:
                gt = self.gt_traj[i] if i < len(self.gt_traj) else self.gt_traj[-1]
            yield imread_unicode(self.img_names[i]), gt
