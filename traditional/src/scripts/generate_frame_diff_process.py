...
x, y, w, h = bbox
cv2.rectangle(diff_colored, (int(x), int(y)), (int(x+w), int(y+h)), (0, 0, 255), 2)
cv2.rectangle(binary_colored, (int(x), int(y)), (int(x+w), int(y+h)), (0, 0, 255), 2)
cv2.rectangle(contour_img, (int(x), int(y)), (int(x+w), int(y+h)), (0, 0, 255), 2)

diff_path = os.path.join(output_dir, "diff_frame.png")
binary_path = os.path.join(output_dir, "binary_frame.png")
contour_path = os.path.join(output_dir, "contour_frame.png")

cv2.imwrite(diff_path, diff_colored)
cv2.imwrite(binary_path, binary_colored)
...
output_path = os.path.join(output_dir, "frame_diff_process.png")
